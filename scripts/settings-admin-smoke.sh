...
cleanup() {
  set +e
  d1 "
    DELETE FROM core_auth_sessions WHERE user_id IN ('$ADMIN_ID','$MANAGER_ID');
    DELETE FROM security_audit_logs WHERE actor_id IN ('$ADMIN_ID','$MANAGER_ID');
    ...
    DELETE FROM auth_users WHERE id IN ('$ADMIN_ID','$MANAGER_ID');
  " >/dev/null 2>&1 || true
}
trap cleanup EXIT
