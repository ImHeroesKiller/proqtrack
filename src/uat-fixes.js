...
function defaultRouteFor(account){
  if(account?.role==='superadmin'||account?.role==='head'||account?.role==='admin'||account?.role==='manager'||account?.role==='supervisor') return '#/';
  return '#/myday';
}
...
function installSessionContinuity(){
  ...
  window.FT.state.user={name:account.name||account.email,role:account.role==='superadmin'?'Superadmin':account.role==='head'?'Head':account.role==='admin'?'Admin':account.role==='manager'?'Manager':account.role==='supervisor'?'Supervisor':'Field Sales',email:account.email};
