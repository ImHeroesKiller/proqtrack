...
function initialDB() {
  return {_version:6,employees:structuredClone(seedEmployees),outlets:structuredClone(seedOutlets),visits:structuredClone(seedVisits),attendance:structuredClone(seedAttendance),accounts:structuredClone(seedAccounts),products:structuredClone(seedProducts),leaveTypes:structuredClone(seedLeaveTypes),leaves:structuredClone(seedLeaves),stocks:structuredClone(seedStocks),priceObservations:structuredClone(seedPriceObservations),competitors:structuredClone(seedCompetitors),competitorProducts:structuredClone(seedCompetitorProducts),competitorIntel:structuredClone(seedCompetitorIntel),promoTypes:structuredClone(seedPromoTypes),fieldPhotos:structuredClone(seedFieldPhotos)};
}
function readDB() { try { return JSON.parse(localStorage.getItem(DB_KEY) || 'null') || initialDB(); } catch { return initialDB(); } }
function saveDB(db) { try { localStorage.setItem(DB_KEY, JSON.stringify(db)); } catch (error) { console.warn('Phase 0 dummy enrichment skipped:', error); } }
function buildEmployees(existing) {
  const rows = Array.isArray(existing) ? [...existing] : [];
  const used = new Set(rows.map(x => x.id));
  for (let i = 0; i < rows.length; i++) {
    const client = CLIENTS[i % CLIENTS.length], project = PROJECTS[i % PROJECTS.length];
  ...
  for (let i = rows.length; i < 100; i++) {
    const n=i+1,name=`${FIRST[i%FIRST.length]} ${LAST[(i*7)%LAST.length]}`,client=CLIENTS[i%CLIENTS.length],project=PROJECTS[i%PROJECTS.length],id=`EMP${String(n).padStart(3,'0')}`;
    if (used.has(id)) continue;
    rows.push({id,name,email:`${name.toLowerCase().replace(/\s+/g,'.')}@proqtrack.id`,phone:`0812-${String(3400+i).padStart(4,'0')}-${String(7000+i).padStart(4,'0')}`,role:i%10===0?'Supervisor':'Field Sales',area:AREAS[i%AREAS.length],status:i%17===0?'inactive':'active',lat:-6.12-((i%12)*.012),lng:106.72+((i%15)*.014),joinDate:`202${2+(i%4)}-${String((i%12)+1).padStart(2,'0')}-${String((i%27)+1).padStart(2,'0')}`,todayVisits:i%8,targetVisits:6+(i%3),totalVisits:45+(i*7)%340,clientId:client[0],projectId:project[0],photo:employeePhoto(name,i)});
  }
