...
\n\n## Performance P2 — runtime responsiveness, network & memory hardening — 25 September 2026\n\nP2 performance menutup jank dan duplicate work setelah aplikasi selesai startup:\n\n- Home, Tracking/Last Location, dan Visits tidak lagi memiliki tiga `setInterval` terpisah. Runtime memakai satu route-aware `setTimeout` scheduler yang hanya hidup pada halaman aktif.\n- Focus dan visibility resume memakai cooldown 5 detik sehingga event browser yang datang berdekatan tidak memicu beberapa refresh untuk revision yang sama.\n- `refreshOperationalData()` memakai single-flight key berdasarkan token, tenant, dan revision; request refresh identik berbagi Promise yang sama.\n- `waitForOperationalSync()` memakai event `proqtrack:cloud-status` di browser dan membersihkan listener/timer setelah selesai, menggantikan polling 50 ms pada runtime browser.\n- Shell render berat dikoaleskan melalui `requestAnimationFrame`; render langsung membatalkan queued frame agar tidak terjadi double paint. Tracking search juga di-debounce 120 ms.\n- Tracking Leaflet map memiliki teardown eksplisit saat meninggalkan route/logout. Outlet map dilepas saat modal ditutup/diganti.\n- Request geocode/search outlet menggunakan `AbortController` untuk membatalkan request stale; reverse geocode memakai bounded cache 50 entry dengan TTL 10 menit.\n- Service Worker memiliki bounded runtime cache per release (`80` entry) dan activation hanya membersihkan namespace cache ProQTrack sehingga cache lain pada origin tidak ikut terhapus.\n- Offline runtime listeners dipasang dengan `AbortController` dan `stopOfflineObserver()` sekarang benar-benar dapat melepas listener serta signature cache.\n- Regression guard utama: `tests/performance-p2.test.mjs`.\n

## Performance P3 — profiling polish, long-session stability & performance budget — 25 September 2026

P3 menutup performance hardening dengan observability lokal, resource budget, dan regression gate:

- Runtime profiler ringan dimuat hanya pada authenticated idle path. Profiler mencatat render duration dan Long Task >=50 ms ke buffer lokal maksimal 60 sample; tidak mengirim telemetry/network request.
- `window.ProQPerformance.snapshot()` menyediakan snapshot lokal untuk debugging: boot duration, render P50/P95, long task, dan JS heap bila browser mendukung `performance.memory`.
- Route render mengirim event `proqtrack:render-complete`; tab background menghentikan route refresh timer dan lifecycle `pagehide` membersihkan timer, map, request/map resources, debounce, dan queued animation frame.
- Offline signature cache untuk perpindahan tenant/superadmin dibatasi maksimal 20 tenant agar sesi panjang tidak menumbuhkan Map tanpa batas.
- `performance-budget.json` menetapkan hard budget untuk hotspot utama (`app.js`, `db.js`, Project Management, cloud data, field sales, index, service worker), total frontend JS, jumlah precache asset, dan total byte precache.
