...
async function resolveEmployeeAccess(env, claims, projectId, requestedEmployeeId = '') {
  ...
  if (!employeeId) {
    const own = await env.DB.prepare(`
      ...
      WHERE e.organization_id=? AND e.auth_user_id=? AND e.employment_status='active'
      LIMIT 1
  ...
  const row = await env.DB.prepare(`
    SELECT e.id,e.auth_user_id,a.supervisor_user_id
    FROM core_employees e
    JOIN core_employee_project_assignments a
      ON a.organization_id=e.organization_id AND a.employee_id=e.id AND a.project_id=? AND a.status='active'
    WHERE e.organization_id=? AND e.id=? AND e.employment_status='active'
    LIMIT 1
...
async function validateRelations(env, claims, projectId, employeeId, outletId, visitId) {
  ...
  if (outletId) {
    const outlet = await env.DB.prepare(`
      SELECT 1 AS ok FROM core_project_outlets
      WHERE organization_id=? AND project_id=? AND outlet_id=? AND status='active' LIMIT 1
  ...
  if (visitId) {
    const visit = await env.DB.prepare(`
      SELECT 1 AS ok FROM core_visits
      WHERE organization_id=? AND id=? AND project_id=? AND employee_id=? LIMIT 1
    `).bind(organizationId, visitId, projectId, employeeId).first();
...
async function handleUpload(request, env, claims, url) {
  ...
  try {
    await validateRelations(env, claims, projectId, employee.id, outletId, visitId);
  } catch (error) {
...
export async function handleEvidenceRoute(request, env, claims, url = new URL(request.url)) {
  ...
  if (suffix.endsWith('/meta') && request.method === 'GET') {
    return handleRead(env, claims, decodeURIComponent(suffix.slice(0, -5)), true);
  }
